...
bool readFileToMatrix (const std::string &filePath, Matrix &mat)
{
  ...
  int matSize = mat.get_cols () * mat.get_rows ();
  long int matBSize = (long int) (matSize * sizeof (float));
  file.seekg (0, std::istream::end);
  long int input_length = file.tellg ();
  file.seekg (0, std::istream::beg);
  if (input_length != matBSize)
    {
      file.close ();
      return false;
    }

  float *matrixData = new float[matSize];

  // read data into the matrix memory
  file.read (reinterpret_cast<char *>(matrixData), matBSize);
